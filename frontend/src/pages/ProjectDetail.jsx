    ...
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6 text-sm text-gray-600">
        <span className="flex items-center gap-1.5">
          <ExternalLink className="w-4 h-4 text-gray-400" /> {project.client_name || 'Sin cliente'}
        </span>
        <span className="flex items-center gap-1.5">
          <GitBranch className="w-4 h-4 text-gray-400" /> {PIPELINE_STAGES.find(s => s.value === project.pipeline_stage)?.label || project.pipeline_stage}
        </span>
        <span className="flex items-center gap-1.5">
          <Tag className="w-4 h-4 text-gray-400" /> {project.billing_type === 'HOURLY' ? 'Por hora' : 'Precio fijo'}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="card">
          ...
          {project.estimated_hours && (
            <p className="text-xs text-gray-400 mt-1">Est: {project.estimated_hours}h</p>
          )}
        </div>
        <div className="card">
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
        ...
        <div className="card">
          ...
          {project.start_date && (
            <p className="text-xs text-gray-400 mt-1">Inicio: {dayjs(project.start_date).format('DD/MM/YYYY')}</p>
          )}
        </div>
      </div>
        ...
        <div className="card mb-8 border-l-4 border-amber-400">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">{unbilled.total_hours || 0}h sin facturar</p>
            </div>
            <p className="text-xl font-bold text-amber-600">{formatCurrency(unbilled.total_amount)}</p>
          </div>
        </div>
      )}
      ...
      <div className="card">
        ...
        {timeEntries.length === 0 ? (
          <p className="text-gray-500 text-sm py-4 text-center">Sin registros de tiempo</p>
        ) : (
            ...
            <table className="w-full">
              ...
              <tbody>
                {timeEntries.map(entry => (
                  <tr key={entry.id} className="border-b border-gray-50 text-sm">
                    <td className="py-2 text-gray-700">{entry.description || '—'}</td>
                    <td className="py-2 text-gray-500">{dayjs(entry.date).format('DD/MM/YYYY')}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tipo facturación</label>
