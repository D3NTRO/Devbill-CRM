    ...
    <div className="p-6 max-w-5xl">
        ...
        {isRunning && runningEntry ? (
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">{runningEntry.project_name || '—'}</p>
              {runningEntry.description && (
                <p className="text-xs text-gray-400">{runningEntry.description}</p>
              )}
            </div>
            <button
              onClick={handleStop}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="card text-center border-amber-200">
          <p className="text-2xl font-bold text-amber-600">{formatDuration(nonBillableMinutes)}</p>
          <p className="text-xs text-gray-500">No facturable</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <button
          ...
          className={`btn btn-secondary flex items-center gap-2 ${showFilters ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : ''}`}
        >
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-3 mb-4 p-4 bg-gray-50 rounded-lg border">
          <div>
            <label className="block text-xs text-gray-500 mb-1">Proyecto</label>
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">Facturable</label>
      ...
      {entries.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-sm text-gray-400 mt-1">Usá el timer o creá una entrada manual</p>
        </div>
      ) : (
            ...
            <tbody>
              {entries.map(entry => (
                <tr key={entry.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="py-3 text-sm font-medium text-gray-900">{entry.project_name}</td>
                  ...
                  <td className="py-3">
                    {entry.is_billable
                      ? <span className="px-2 py-0.5 text-xs rounded-full bg-green-50 text-green-700">Sí</span>
                      : <span className="px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-500">No</span>
                    }
                  </td>
                  <td className="py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(entry)} className="p-1.5 text-gray-400 hover:text-indigo-600 rounded" title="Editar">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => setDeleting(entry)} className="p-1.5 text-gray-400 hover:text-red-600 rounded" title="Eliminar">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="is_billable" className="text-sm text-gray-700">Facturable</label>
              </div>
